...
int get_local_mac(const char *ifname, char *mac)
{
    ...
    if( (sd = socket(AF_INET, SOCK_STREAM, 0)) < 0) {
        EMBER_LOGE("get %s mac address socket creat error", ifname);
        return -EMBER_ERROR;
    }
    ...
    if(ioctl(sd, SIOCGIFHWADDR, &ifr) < 0) {
        EMBER_LOGE("get %s mac address error", ifname);
        close(sd);
        return -EMBER_ERROR;
    }
...
int set_local_mac(const char *ifname, const char *mac)
{
    ...
    if(6 > ret) {
        EMBER_LOGE("invalid mac address");
        return -EMBER_ERROR;
    }
...
int get_local_ip(const char *ifname, char *ip)
{
    ...
    if (-1 == sd) {
        EMBER_LOGE("socket error: %s", strerror(errno));
        return -EMBER_ERROR;		
    }
    ...
    if (ioctl(sd, SIOCGIFADDR, &ifr) < 0) {
        EMBER_LOGE("ioctl error: %s", strerror(errno));
        close(sd);
        return -EMBER_ERROR;
    }
...
int get_local_netmask(const char *ifname, char *netmask_addr)
{
    ...
    if ((ioctl(sd, SIOCGIFNETMASK, &ifr)) < 0 ) {
        EMBER_LOGE("netmask ioctl error");
        close(sd);
        return -EMBER_ERROR;
    }
...
int set_local_netmask(const char *ifname, const char *netmask_addr)
{
    ...
    if (ioctl(sd, SIOCSIFNETMASK, &ifr) < 0) {
        EMBER_LOGE("sock_netmask ioctl error");
        close(sd);
        return -EMBER_ERROR;
    }
...
link_status_t get_netdev_status(const char *ifname)
{
    ...
    if (ioctl(skfd, SIOCETHTOOL, &ifr) == -1) {
        EMBER_LOGE("ETHTOOL_GLINK failed: %s", strerror(errno));
        close(skfd);
        return -EMBER_ERROR;
    }
...
int set_netdev_status(const char *ifname, const link_status_t status)
{
    ...
    if ((sockfd = socket(AF_INET, SOCK_STREAM, 0)) < 0) {
        EMBER_LOGE("Create socket failed");
        return -EMBER_ERROR;
    }
    
    strcpy(ifr.ifr_name, ifname);
    if (ioctl(sockfd, SIOCGIFFLAGS, &ifr) < 0) {
        EMBER_LOGE("ioctl SIOCGIFFLAGS failed");
        close(sockfd);
        return -EMBER_ERROR;
    }
    ...
    if (ioctl(sockfd, SIOCSIFFLAGS, &ifr) < 0) {
        EMBER_LOGE("ioctl SIOCSIFFLAGS failed");
        close(sockfd);
        return -EMBER_ERROR;
    }
